...
public class OneXTwoMapper implements MarketMapper{
    ...
    public OutMarket map(InMarket inMarket) {
        ...
        for (InSelection inSelection : inMarket.getSelections()) {
            ...
            OutSelection outSelection = getOneXTwoOutSelection(inSelection, selectionName, marketUid);
    ...
    private static OutSelection getOneXTwoOutSelection(InSelection inSelection, String selectionName, String marketUid) {
        int id = switch (selectionName) {
            case "team a" -> SelectionType.TEAM_A.id;
            case "draw" -> SelectionType.DRAW.id;
            case "team b" -> SelectionType.TEAM_B.id;
            default -> throw new IllegalArgumentException("Unknown 1X2 selection name: " + selectionName);
        };
